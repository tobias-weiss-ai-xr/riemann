...
\section{Brody $\beta$ Analysis by Dimension}
\label{sec:exp15}
...
\textbf{dim=1} & \textbf{34,628} & \textbf{227,043} & \textbf{1.879} & \textbf{[1.870, 1.888]} \\
\textbf{dim$\geq$2} & \textbf{29,216} & \textbf{262,944} & \textbf{0.242} & \textbf{[0.238, 0.246]} \\ \midrule
...
\section{Continuous Dimension Dependence}
\label{sec:exp16}
...
\section{Prediction of Zero Spacings from Hecke Traces}
\label{sec:task3}
...
\begin{tabular}{lcc}
\toprule
Target & $R^2$ (Traces) & $R^2$ (Scalars) \\ \midrule
mean\_spacing & 0.281 & 0.943 \\
std\_spacing & 0.129 & 0.948 \\
mean\_zero\_spacing & --- & 0.906 \\ \bottomrule
\end{tabular}
\end{table}

\paragraph{Key Finding 4}: Scalar metadata alone determines spacing statistics with high accuracy. Traces alone achieve only $R^2=0.13-0.28$, but scalar features alone achieve $R^2=0.94-0.95$.
...
\section{Predicting Spectral Rigidity from Metadata}
\label{sec:task5}
...
The complete dataset (63,844 weight-2 newforms with 10 lowest zeros and 100 Hecke eigenvalues each), all analysis code, and the figures are archived on Zenodo~\cite{zenodo2026}.
